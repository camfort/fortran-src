...
analyseRenames (ProgramFile mi pus) = ProgramFile mi pus'
 where
  (Just pus', _) =
    runRenamer (skimProgramUnits pus >> renameSubPUs (Just pus)) (renameState0 (miVersion mi))

-- | Annotate unique names for variable and function declarations and uses. With external module map.
analyseRenamesWithModuleMap :: Data a
                            => ModuleMap
...
unrename = trPU fPU . trE fE
 where
  ...
  trPU = transformBi
  fPU :: Data a => ProgramUnit (Analysis a) -> ProgramUnit (Analysis a)
  fPU (PUFunction a s ty r _ args res b subs) | Just srcN <- sourceName a =
    PUFunction a s ty r srcN args res b subs
  fPU (PUSubroutine a s r _ args b subs) | Just srcN <- sourceName a =
    PUSubroutine a s r srcN args b subs
  fPU pu = pu
...
declarator (DeclArray a s e1 ddAList me2 me3) = do
  e1' <- renameExpDecl e1
  let trans :: RenamerFunc (Expression (Analysis a))
            -> RenamerFunc (AList DimensionDeclarator (Analysis a))
      trans = transformBiM
...
pushScope name env0 =
  modify $ \s -> s { scopeStack = name : scopeStack s, environ = env0 : environ s }

-- Pop a scope from the lexical stack.
popScope :: Renamer ()
popScope = modify $ \s -> s { scopeStack = drop 1 $ scopeStack s, environ = drop 1 $ environ s }
...
addToEnv v v' nt =
  modify $ \s -> s { environ = insert v (v', nt) (head (environ s)) : drop 1 (environ s) }
...
setUniqueName un x
  | a@Analysis { uniqueName = Nothing } <- getAnnotation x = setAnnotation
    (a { uniqueName = Just un })
    x
  | otherwise = x

setSourceName sn x
  | a@Analysis { sourceName = Nothing } <- getAnnotation x = setAnnotation
    (a { sourceName = Just sn })
    x
  | otherwise = x
...
renameGenericDecls = trans renameExpDecl
 where
  trans :: (Data a, Data (f (Analysis a)))
...
renameExpDecl e@(ExpValue _ _ (ValVariable v)) =
  flip setUniqueName (setSourceName v e) `fmap` maybeAddUnique v NTVariable
-- Intrinsics get unique names for each use.
renameExpDecl e@(ExpValue _ _ (ValIntrinsic v)) =
  flip setUniqueName (setSourceName v e) `fmap` addUnique v NTIntrinsic
...
renameExp e@(ExpValue _ _ (ValVariable v)) =
  maybe e (`setUniqueName` setSourceName v e) `fmap` getFromEnvs v
-- Intrinsics get unique names for each use.
renameExp e@(ExpValue _ _ (ValIntrinsic v)) =
  flip setUniqueName (setSourceName v e) `fmap` addUnique v NTIntrinsic
