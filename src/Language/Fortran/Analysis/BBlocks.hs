...
labelBlocksInBBGr = transform (nmapM' (mapM eachBlock))
 where
  ...
  transform :: Data a
            => (BBGr a -> State Int (BBGr a))
            -> ProgramFile a
            -> State Int (ProgramFile a)
  transform = transformBiM
...
labelExprsInBBGr = transformBB (nmapM' (transformExpr eachExpr))
 where
  ...
  transformExpr :: Data a
                => (Expression (Analysis a) -> State Int (Expression (Analysis a)))
                -> [Block (Analysis a)]
                -> State Int [Block (Analysis a)]
...
insEntryEdges :: (Data a, DynGraph gr)
              => ProgramUnit (Analysis a)
              -> gr [Block (Analysis a)] ()
              -> gr [Block (Analysis a)] ()
insEntryEdges pu = insEdge (0, 1, ()) . insNode (0, bs) where bs = genInOutAssignments pu False
...
insExitEdges :: (Data a, DynGraph gr)
             => ProgramUnit (Analysis a)
             -> M.Map String Node
             -> gr [Block (Analysis a)] ()
             -> gr [Block (Analysis a)] ()
insExitEdges pu lm gr = flip insEdges (insNode (-1, bs) gr) $ do
  ...
  return (n, n', ())
  where bs = genInOutAssignments pu True
...
examineFinalBlock lm bs@(_ : _)
  | BlStatement _ _ _ (StGotoUnconditional _ _ k) <- last bs
  = [lookupBBlock lm k]
  | BlStatement _ _ _ (StGotoAssigned _ _ _ ks) <- last bs
  = map (lookupBBlock lm) (maybe [] aStrip ks)
  | BlStatement _ _ _ (StGotoComputed _ _ ks _) <- last bs
  = map (lookupBBlock lm) (aStrip ks)
  | BlStatement _ _ _ StReturn{} <- last bs
  = [-1]
...
isFinalBlockCtrlXfer bs@(_ : _) | BlStatement _ _ _ StGotoUnconditional{} <- last bs = True
...
processBlocks bs = do
  ...
  mapM_ perBlock bs
  endN <- gets curNode
  modify $ \st -> st { bbGraph = insNode (endN, reverse (curBB st)) (bbGraph st), curBB = [] }
  return (startN, endN)
...
perBlock b@(BlDo      _ _ _ _ _ (Just spec) bs _) = do
  ...
  _ <- case me3 of
    Just e3 -> Just `fmap` processFunctionCalls e3
...
perBlock b@(BlStatement _ _ _ StReturn{}        ) = processLabel b >> addToBBlock b >> closeBBlock_
...
perBlock (  BlStatement a s l (StCall a' s' cn@ExpValue{} (Just aargs))) = do
  ...
  forM_ (zip exps [(1 :: Integer) ..]) $ \(e, i) -> do
    ...
    addToBBlock . analyseAllLhsVars1 $ BlStatement a
                                                   s
                                                   Nothing
                                                   (StExpressionAssign a' s' (formal e' i) e')
  ...
  forM_ (zip exps [(1 :: Integer) ..]) $ \(e, i) ->
    ...
    (when (isLExpr e) $ addToBBlock . analyseAllLhsVars1 $ BlStatement
      a
      s
      Nothing
      (StExpressionAssign a' s' e (formal e i))
    )
  (_, nextN) <- closeBBlock
...
perDoBlock repeatExpr b bs = do
  ...
  case getLabel b of
    ...
    _                                  -> return ()
  case repeatExpr of
    Just e  -> void (processFunctionCalls e)
    Nothing -> return ()
  addToBBlock $ stripNestedBlocks b
  _              <- closeBBlock
...
stripNestedBlocks (BlStatement a s l (StIfLogical a' s' e _)) =
  BlStatement a s l (StIfLogical a' s' e (StEndif a' s' Nothing))
...
processFunctionCall (ExpFunctionCall a s fn@(ExpValue a' s' _) aargs) = do
  ...
  let setName n e =
        setAnnotation ((getAnnotation e) { uniqueName = Just n, sourceName = Just n }) e
  let formal (ExpValue _ s'' (ValVariable _)) i = setName n $ ExpValue a0 s'' (ValVariable n)
      ...
      formal e i = setName n $ ExpValue a0 s'' (ValVariable n)
       where
        s'' = getSpan e
        n   = name i
  forM_ (zip exps [(1 :: Integer) ..]) $ \(e, i) -> addToBBlock . analyseAllLhsVars1 $ BlStatement
    a0
    s
    Nothing
    (StExpressionAssign a' s' (formal e i) e)
  ...
  addToBBlock . analyseAllLhsVars1 $ BlStatement a
                                                 s
                                                 Nothing
                                                 (StCall a' s' fn (Just $ fromList a0 dummyArgs))
  (_, returnedN) <- closeBBlock
...
genSuperBBGr bbm = SuperBBGr { graph = superGraph'', clusters = cmap, entries = entryMap }
 where
  ...
  stCalls =
    [ (getSuperNode n, sub)
    | (n, [BlStatement _ _ _ (StCall _ _ e _)]) <- namedNodes
    , v@ExpValue{}                              <- [e]
    , let sub = varName v
  ...
  stCallEdges = concat
    [ [ (m, nEn, l) | (m, _, l) <- inEdges ] ++ [ (nEx, m, l) | (_, m, l) <- outEdges ]
...
findLabeledBBlock llab gr = listToMaybe
  [ n
  | (n, bs)                         <- labNodes gr
  , b                               <- bs
...
showAnalysedBBGr = showBBGr . nmap strip where strip = map (fmap insLabel)
...
bbgrToDOT' clusters' gr = execWriter $ do
  ...
  _ <- forM nodes' $ \n -> do
    ...
    case mname of
      Just name -> do
        tell $ "subgraph \"cluster " ++ showPUName name ++ "\" {\n"
...
showBlock (BlStatement _ _ mlab st) | null (str :: String) = ""
 ...
 where
  str = case st of
    ...
    StDeclaration _ _ ty Nothing adecls    -> showType ty ++ " " ++ aIntercalate ", " showDecl adecls
    StDeclaration _ _ ty (Just aattrs) adecls ->
      showType ty ++ " " ++ aIntercalate ", " showAttr aattrs ++ aIntercalate ", " showDecl adecls
...
showExpr (ExpSubscript _ _ e1 aexps) =
  showExpr e1 ++ "[" ++ aIntercalate ", " showIndex aexps ++ "]"
showExpr _ = ""
...
showDim (DimensionDeclarator _ _ me1 me2) =
  maybe "" ((++ ":") . showExpr) me1 ++ maybe "" showExpr me2
