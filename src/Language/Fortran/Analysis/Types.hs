...
analyseTypesWithEnv
  :: Data a => TypeEnv -> ProgramFile (Analysis a) -> (ProgramFile (Analysis a), TypeEnv)
  ...
  _   <- forM eps $ \(eName, (fName, mRetName)) -> do
    ...
    case mFType of
      Just (IDType fVType fCType) -> do
        ...
        maybe
          (return ())
          (error "Entry points with result variables unsupported" >> recordMType fVType Nothing)
          mRetName
      _ -> return ()
...
extractTypeEnv pf = M.union puEnv expEnv
 where
  puEnv = M.fromList
    [ (n, ty)
    | pu      <- universeBi pf :: [ProgramUnit (Analysis a)]
    , Named n <- [puName pu]
    , ty      <- maybeToList (idType (getAnnotation pu))
    ]
  expEnv = M.fromList
    [ (n, ty)
    | e <- universeBi pf :: [Expression (Analysis a)]
    , let n = varName e
...
annotateTypes pf =
  (transformBiM :: Data a => TransType Expression ProgramFile a) annotateExpression pf
...
programUnit pu@(PUFunction _ _ mRetType _ _ _ mRetVar blocks _) | Named n <- puName pu = do
    -- record some type information that we can glean
  recordCType CTFunction n
  case (mRetType, mRetVar) of
    (Just (TypeSpec _ _ baseType _), Just v) ->
      recordBaseType baseType n >> recordBaseType baseType (varName v)
    (Just (TypeSpec _ _ baseType _), _) -> recordBaseType baseType n
    _ -> return ()
  -- record entry points for later annotation
  forM_ blocks $ \block -> sequence_
    [ recordEntryPoint n (varName v) (fmap varName mRetVar')
    | (StEntry _ _ v _ mRetVar') <- allStatements block
    ]
programUnit pu@(PUSubroutine _ _ _ _ _ blocks _) | Named n <- puName pu = do
  ...
  -- record entry points for later annotation
  forM_ blocks $ \block -> sequence_
    [ recordEntryPoint n (varName v) Nothing | (StEntry _ _ v _ _) <- allStatements block ]
programUnit _ = return ()

declarator :: Data a => InferFunc (Declarator (Analysis a))
declarator (DeclArray _ _ v ddAList _ _) =
  recordCType (CTArray $ dimDeclarator ddAList) (varName v)
declarator _ = return ()

dimDeclarator :: AList DimensionDeclarator a -> [(Maybe Int, Maybe Int)]
dimDeclarator ddAList =
  [ (lb, ub)
  | DimensionDeclarator _ _ lbExp ubExp <- aStrip ddAList
  , let lb = do
          ExpValue _ _ (ValInteger i) <- lbExp
          return $ read i
  , let ub = do
          ExpValue _ _ (ValInteger i) <- ubExp
          return $ read i
  ]
...
statement (StDeclaration _ _ (TypeSpec _ _ baseType _) mAttrAList declAList)
  ...
  = do
    ...
    let cType n | isExtrn   = CTExternal
                ...
                | isParam   = CTParameter
                | Just (IDType _ (Just ct)) <- M.lookup n env, ct /= CTIntrinsic = ct
    ...
    let bType (Just e) | TypeCharacter _ kind <- baseType = TypeCharacter (Just $ charLen e) kind
                       | otherwise                        = TypeCharacter (Just $ charLen e) Nothing
        bType Nothing = baseType
    forM_ decls $ \decl -> case decl of
      DeclArray _ _ v ddAList e _ ->
        recordType (bType e) (CTArray $ dimDeclarator ddAList) (varName v)
...
statement (StExpressionAssign _ _ (ExpFunctionCall _ _ v Nothing) _) =
...
annotateExpression e@(ExpValue _ _ (ValVariable _)) =
  maybe e (`setIDType` e) `fmap` getRecordedType (varName e)
annotateExpression e@(ExpValue _ _ (ValIntrinsic _)) =
  maybe e (`setIDType` e) `fmap` getRecordedType (varName e)
...
recordType bt ct n =
  modify $ \s -> s { environ = insert n (IDType (Just bt) (Just ct)) (environ s) }
...
recordEntryPoint fn en mRetName =
  modify $ \s -> s { entryPoints = M.insert en (fn, mRetName) (entryPoints s) }
...
setIDType ty x | a@Analysis{} <- getAnnotation x = setAnnotation (a { idType = Just ty }) x
               | otherwise                       = x
