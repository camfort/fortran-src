...
genModGraph :: Maybe FortranVersion -> [FilePath] -> Bool -> [FilePath] -> IO ModGraph
genModGraph mversion includeDirs useCPP paths = do
  ...
  let iter :: FilePath -> ModGrapher ()
      iter path = do
        contents <- liftIO $ if useCPP then runCPP path else flexReadFile path
