...
genericGroup groupingFunction = modifyProgramFile $ transformBi groupingFunction
 ...
 where
  (b', bs') = case b of
    BlStatement a s label st
      ...
      -> let (blocks, leftOverBlocks, endLabel) = collectNonForallBlocks groupedBlocks mTarget
         in  (BlForall a (getTransSpan s blocks) label mTarget header blocks endLabel, leftOverBlocks)
      | StForallStatement _ _ header st' <- st
      -> let block = BlStatement a (getSpan st') Nothing st'
         in  (BlForall a (getTransSpan s st') label Nothing header [block] Nothing, groupedBlocks)
    b'' | containsGroups b'' -> (applyGroupingToSubblocks groupForall' b'', groupedBlocks)
    _                        -> (b, groupedBlocks)
  groupedBlocks = groupForall' bs

collectNonForallBlocks
  :: ABlocks a -> Maybe String -> (ABlocks a, ABlocks a, Maybe (Expression (Analysis a)))
collectNonForallBlocks blocks mNameTarget = case blocks of
  BlStatement _ _ mLabel (StEndForall _ _ mName) : rest
    | mName == mNameTarget -> ([], rest, mLabel)
    | otherwise            -> error "Forall block name does not match that of the end statement."
  b : bs ->
    let (bs', rest, mLabel) = collectNonForallBlocks bs mNameTarget in (b : bs', rest, mLabel)
...
groupIf' (b : bs) = b' : bs'
 where
  (b', bs') = case b of
    BlStatement a s label st | StIfThen _ _ mName _ <- st -> -- If statement
      let (conditions, blocks, leftOverBlocks, endLabel) = decomposeIf (b : groupedBlocks)
      in  (BlIf a (getTransSpan s blocks) label mName conditions blocks endLabel, leftOverBlocks)
...
decomposeIf
  :: ABlocks a
  -> ([Maybe (Expression (Analysis a))], [ABlocks a], ABlocks a, Maybe (Expression (Analysis a)))
decomposeIf blocks@(BlStatement _ _ _ (StIfThen _ _ mTargetName _) : _) = decomposeIf' blocks
 where
  decomposeIf' (BlStatement _ _ mLabel st : rest) = case st of
    ...
    StEndif _ _ mName
      ...
      | otherwise
      -> error
        $  "If statement name does not match that of "
  ...
  go maybeCondition blocks' =
    ...
    in  (maybeCondition : conditions, nonConditionBlocks : listOfBlocks, rest'', endLabel)
decomposeIf _ = error "can't decompose block"

-- This compiles the executable blocks under various if conditions.
collectNonConditionalBlocks :: ABlocks a -> (ABlocks a, ABlocks a)
collectNonConditionalBlocks blocks = case blocks of
 ...
 where
  (b', bs') = case b of
    BlStatement a s label st
      ...
      -> let (blocks, leftOverBlocks, endLabel) = collectNonDoBlocks groupedBlocks mTarget
      ...
      -> let (blocks, leftOverBlocks, endLabel) = collectNonDoBlocks groupedBlocks mName
         in  ( BlDo a (getTransSpan s blocks) label mName Nothing doSpec blocks endLabel
             , leftOverBlocks
             )
    b'' | containsGroups b'' -> (applyGroupingToSubblocks groupDo' b'', groupedBlocks)
    _                        -> (b, groupedBlocks)
  groupedBlocks = groupDo' bs -- Assume everything to the right is grouped.

collectNonDoBlocks
  :: ABlocks a -> Maybe String -> (ABlocks a, ABlocks a, Maybe (Expression (Analysis a)))
collectNonDoBlocks blocks mNameTarget = case blocks of
  BlStatement _ _ mLabel (StEnddo _ _ mName) : rest
    | mName == mNameTarget -> ([], rest, mLabel)
    | otherwise            -> error "Do block name does not match that of the end statement."
  b : bs -> let (bs', rest, mLabel) = collectNonDoBlocks bs mNameTarget in (b : bs', rest, mLabel)
...
groupLabeledDo' (b : bs) = b' : bs'
 where
  (b', bs') = case b of
    BlStatement a s label (StDo _ _ mn tl@Just{} doSpec) ->
      let (blocks, leftOverBlocks) = collectNonLabeledDoBlocks tl groupedBlocks
          lastLabel                = getLastLabel $ last blocks
      in  (BlDo a (getTransSpan s blocks) label mn tl doSpec blocks lastLabel, leftOverBlocks)
    BlStatement a s label (StDoWhile _ _ mn tl@Just{} cond) ->
      let (blocks, leftOverBlocks) = collectNonLabeledDoBlocks tl groupedBlocks
          lastLabel                = getLastLabel $ last blocks
      in  (BlDoWhile a (getTransSpan s blocks) label mn tl cond blocks lastLabel, leftOverBlocks)
    b'' | containsGroups b'' -> (applyGroupingToSubblocks groupLabeledDo' b'', groupedBlocks)
    _                        -> (b, groupedBlocks)
...
collectNonLabeledDoBlocks :: Maybe (Expression (Analysis a)) -> ABlocks a -> (ABlocks a, ABlocks a)
collectNonLabeledDoBlocks targetLabel blocks = case blocks of
  ...
  b : bs
    ...
    | otherwise -> let (bs', rest) = collectNonLabeledDoBlocks targetLabel bs in (b : bs', rest)

compLabel :: Maybe (Expression a) -> Maybe (Expression a) -> Bool
compLabel (Just (ExpValue _ _ (ValInteger l1))) (Just (ExpValue _ _ (ValInteger l2))) =
  strip l1 == strip l2
compLabel _ _ = False
...
groupCase' (b : bs) = b' : bs'
 where
  (b', bs') = case b of
    BlStatement a s label st | StSelectCase _ _ mName scrutinee <- st ->
...
decomposeCase (BlStatement _ _ mLabel st : rest) mTargetName = case st of
  StCase _ _ mName mCondition
    ...
    -> go mCondition rest
    | otherwise
    -> error $ "Case name does not match that of " ++ "the corresponding select case statement."
  StEndcase _ _ mName
    | mName == mTargetName
    -> ([], [], rest, mLabel)
    | otherwise
    -> error $ "End case name does not match that of " ++ "the corresponding select case statement."
 ...
 where
  go mCondition blocks =
    ...
    in  (mCondition : conditions, nonCaseBlocks : listOfBlocks, rest'', endLabel)
decomposeCase _ _ = error "can't decompose case"

-- This compiles the executable blocks under various if conditions.
collectNonCaseBlocks :: ABlocks a -> (ABlocks a, ABlocks a)
collectNonCaseBlocks blocks = case blocks of
  BlStatement _ _ _ st : _ | StCase{} <- st    -> ([], blocks)
                           | StEndcase{} <- st -> ([], blocks)
...
applyGroupingToSubblocks f b
  | BlStatement{} <- b = error "Individual statements do not have subblocks. Must not occur."
  | BlIf a s l mn conds blocks el <- b = BlIf a s l mn conds (map f blocks) el
  | BlCase a s l mn scrutinee conds blocks el <- b = BlCase a
                                                            s
                                                            l
                                                            mn
                                                            scrutinee
  ...
  | BlInterface{} <- b = error "Interface blocks do not have groupable subblocks. Must not occur."
  | BlComment{} <- b = error "Comment statements do not have subblocks. Must not occur."
  | BlForall a s ml mn h blocks mel <- b = BlForall a s ml mn h (f blocks) mel
