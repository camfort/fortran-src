...
import           Language.Fortran.ParserMonad             ( selectFortranVersion
                                                          , FortranVersion(..)
                                                          , fromRight
                                                          )
import qualified Language.Fortran.Lexer.FixedForm
                                               as FixedForm
                                                          ( collectFixedTokens
                                                          , Token(..)
                                                          )
import qualified Language.Fortran.Lexer.FreeForm
                                               as FreeForm
                                                          ( collectFreeTokens
                                                          , Token(..)
                                                          )
  ...
  case (parsedArgs, action opts) of
    ([path], actionOpt) -> do
      ...
      let runSuperGraph pf | outfmt == DOT = superBBGrToDOT sgr
           ...
           where
            pf' = analyseBBlocks . analyseRenamesWithModuleMap mmap . initAnalysis $ pf
            bbm = genBBlockMap pf'
...
superGraphDataFlow :: forall a
                    . (Out a, Data a)
                   => ProgramFile (Analysis a)
                   -> SuperBBGr (Analysis a)
                   -> String
superGraphDataFlow pf sgr =
  showBBGr (nmap (map (fmap insLabel)) gr')
    ++ "\n\n"
    ++ replicate 50 '-'
    ++ "\n\n"
    ++ show entries
  ...
  dfStr gr =
    (\(l, x) -> '\n' : l ++ ": " ++ x)
      =<< [ ("callMap"     , show cm)
          ...
          , ( "blockMap"
            , unlines [ "AST-block " ++ show i ++ ":\n" ++ pretty b | (i, b) <- IM.toList bm ]
            )
          , ( "derivedInd"
            , unlines
              [ "Expression " ++ show i ++ " (IE: " ++ show ie ++ "):\n" ++ pretty e
...
showModuleMap =
  concatMap (\(n, m) -> show n ++ ":\n" ++ (unlines . map ("  " ++) . lines . showGenericMap $ m))
    . M.toList
showTypes :: TypeEnv -> String
showTypes tenv =
  flip concatMap (M.toList tenv) $ \(name, IDType { idVType = vt, idCType = ct }) ->
...
options =
  ...
  , Option ['R'] ["rename"]  (NoArg $ \opts -> opts { action = Rename })  "parse and rename variables"
  , Option ['B'] ["bblocks"] (NoArg $ \opts -> opts { action = BBlocks }) "analyse basic blocks"
  , Option ['S']
           ["supergraph"]
