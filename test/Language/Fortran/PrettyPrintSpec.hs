...
checkAll :: forall a b c
          . (Out c, Data c, Data a, Data b)
         => (b -> Maybe c)
         -> (c -> Spec)
         -> a
         -> Spec
checkAll restrict check t = describe ("Testing on " ++ show (length inputs) ++ " nodes")
  $ mapM_ check inputs
  where inputs = catMaybes [ restrict b | b <- universeBi t :: [b] ]

samplesBase :: FilePath
samplesBase = "test" </> "Language" </> "Fortran" </> "samples"

spec :: Spec
spec = describe "Pretty printer tests" $ do
  ...
  describe "Format item" $ it "prints hollerith constant" $ do
  ...
  describe "Statement" $ do
    describe "Declaration" $ do
      it "prints 90 style with attributes" $ do
        ...
        let st     = StDeclaration () u typeSpec (Just $ AList () u attrs) (AList () u declList)
      ...
      it "prints 77 style" $ do
        ...
        let declList = [DeclArray () u (varGen "x") (AList () u dds) Nothing Nothing]
        let st       = StDeclaration () u typeSpec Nothing (AList () u declList)
        pprint Fortran77 st Nothing `shouldBe` "integer x(10)"

    describe "Intent" $ it "prints intent statement" $ do
    ...
    describe "Equivalence" $ it "prints multiple equivalence groups" $ do
      let equivGroups = [AList () u [varGen "x", varGen "y"], AList () u [varGen "z"]]
      let st          = StEquivalence () u (AList () u equivGroups)
      pprint Fortran90 st Nothing `shouldBe` "equivalence (x, y), (z)"

    describe "Equivalence" $ it "prints entry point with arguments and result specified" $ do
    ...
    describe "If" $ do
      it "prints arithmetic if" $ do
        let arIf = StIfArithmetic () u (intGen 0) (intGen 10) (intGen 20) (intGen 30)
    ...
    describe "Function statement" $ it "prints function statement" $ do
    ...
    describe "Stop"
      ...
      $          pprint Fortran66 (StStop () u (Just $ intGen 1)) Nothing
      `shouldBe` "stop 1"

    describe "IO" $ describe "Print" $ it "prints vanilla print statement" $ do
      let st = StPrint () u starVal (Just $ AList () u [intGen 42])
      pprint Fortran90 st Nothing `shouldBe` "print *, 42"

    describe "Allocation" $ describe "Allocate" $ it "prints allocate statement" $ do
    ...
    describe "Where" $ it "prints statement" $ do
  ...
  describe "Blocks" $ do
    describe "Comment" $ do
      ...
      it "prints 90 style comment"
        $          pprint Fortran90 blComment Nothing
        `shouldBe` "! si vis pacem para bellum\n"

      it "prints 66 style comment"
        $          pprint Fortran66 blComment Nothing
        `shouldBe` "c si vis pacem para bellum\n"

    describe "Statement" $ it "prints vanilla print" $ do
      let st = StPrint () u starVal Nothing
      let bl = BlStatement () u (Just $ intGen 42) st
      pprint Fortran90 bl Nothing `shouldBe` "42 print *\n"

    describe "Interface" $ it "prints interface block" pending
    ...
    describe "Do While" $ do
      it "prints simple do while loop" $ do
        ...
        let
          expect = unlines
            ["my_block: do while ((i > 42))", "print *, i", "i = (i - 1)", "end do my_block"]
      ...
      it "prints a labelled do while loop" $ do
        ...
        let expect = unlines ["do 10 while ((i > 42))", "print *, i", "i = (i - 1)"]
        pprint Fortran90 bl Nothing `shouldBe` text expect
    ...
    describe "Do" $ do
      ...
      it "prints 90 style do loop" $ do
        ...
        let expect = unlines ["do i = 1, 9, 2", "print *, i", "i = (i - 1)", "end do"]
        pprint Fortran90 bl Nothing `shouldBe` text expect

      it "prints named infinite do loop" $ do
        let bl     = BlDo () u Nothing (Just "joker") Nothing Nothing body Nothing
        let expect = unlines ["joker: do", "print *, i", "i = (i - 1)", "end do joker"]
        pprint Fortran90 bl Nothing `shouldBe` text expect
      ...
      it "prints named labeled do loop" $ do
        let
          bl = BlDo ()
                    ...
                    (Just $ intGen 42)
        let expect =
              unlines ["joker: do 42 i = 1, 9, 2", "print *, i", "i = (i - 1)", "42 end do joker"]
        pprint Fortran90 bl Nothing `shouldBe` text expect

      it "prints vanilla labeled do loop" $ do
        let body2 = body ++ [BlStatement () u (Just $ intGen 42) (StContinue () u)]
        let bl =
              BlDo () u Nothing Nothing (Just $ intGen 42) (Just doSpec) body2 (Just $ intGen 42)
        let expect =
              unlines
                [ "      do 42 i = 1, 9, 2"
                , "        print *, i"
    ...
    describe "If" $ do
      it "prints vanilla structured if" $ do
        ...
        let expect = unlines ["if (.true.) then", "print *, i", "i = (i - 1)", "end if"]
        pprint Fortran90 bl Nothing `shouldBe` text expect
    ...
    describe "Case" $ it "prints complicated structured if" $ do
      let range = IxRange () u (Just $ intGen 2) (Just $ intGen 4) Nothing
      let
        cases =
          [Just (AList () u [range]), Just (AList () u [IxSingle () u Nothing (intGen 7)]), Nothing]
  ...
  describe "Program units" $ do
    ...
    describe "Module" $ do
      it "prints module without sub programs" $ do
        ...
        let expect =
              unlines ["module my_mod", "  print *, i", "  i = (i - 1)", "end module my_mod"]
    ...
    describe "Subroutine" $ do
      it "prints recursive subroutine with args without sub programs" $ do
        ...
        let expect =
              unlines
                [ "recursive subroutine sub(x, y, z)"
                , "print *, i"
                , "i = (i - 1)"
                , "end subroutine sub"
                ]
      ...
      it "prints 66 style subroutine without args" $ do
        ...
        let
          expect = unlines
            ["      subroutine sub", "        print *, i", "        i = (i - 1)", "      end"]
  ...
  describe "Program file" $ it "prints simple program file" $ do
