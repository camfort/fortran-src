    ...
    describe "Statement" $ do
      describe "Declaration" $ do
        it "prints 90 style with attributes" $ do
          ...
          let declList =
                [ Declarator () u (varGen "x") Nothing Nothing (Just $ intGen 42)
                , Declarator () u (varGen "y") Nothing (Just $ intGen 3) Nothing ]
        ...
        it "prints 77 style" $ do
          ...
          let declList =
                [ Declarator () u (varGen "x") (Just (AList () u dds)) Nothing
      ...
      describe "Parameter" $
        it "prints vanilla statement" $ do
          let decls = [ Declarator () u (varGen "x") Nothing Nothing (Just $ intGen 42)
                      , Declarator () u (varGen "y") Nothing Nothing (Just $ intGen 24)
                      ]
