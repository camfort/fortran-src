...
spec :: Spec
spec = describe "Fortran 77 Parser" $ do
  describe "IO" $ do
    ...
    it "parses 'endfile i" $ sParser "      endfile i" `shouldBe'` StEndfile2 () u (varGen "i")
    ...
    it "parses 'read *, (x, y(i), i = 1, 10, 2)'" $ do
      let stAssign      = StExpressionAssign () u (varGen "i") (intGen 1)
          ...
          impliedDoVars = AList
            ...
            u
            [ varGen "x"
            , ExpSubscript () u (varGen "y") (AList () u [IxSingle () u Nothing $ varGen "i"])
            ]
  ...
  describe "Subscript like" $ do
    it "parses vanilla subscript" $ do
      let exp = ExpSubscript
            ...
            (AList
              ()
              u
              [ IxSingle () u Nothing $ varGen "x"
              , IxSingle () u Nothing $ intGen 2
              , IxSingle () u Nothing $ intGen 3
              ]
            )
    ...
    it "parses array declarator" $ do
      let dimDecls =
            [ DimensionDeclarator () u (Just $ intGen 1)   (Just $ intGen 2)
  ...
  describe "GOTO" $ do
    ...
    let gotoSt = StGotoAssigned ()
                                u
                                (varGen "v")
                                (Just (AList () u [labelGen 10, labelGen 20, labelGen 30]))
    it "parses assigned GOTO with comma" $ sParser "      GOTO v, (10, 20, 30)" `shouldBe'` gotoSt

    it "parses assigned GOTO without comma" $ sParser "      GOTO v (10, 20, 30)" `shouldBe'` gotoSt
    ...
    it "parses 'implicit character*30 (a, b, c), integer (a-z, l)" $ do
      let impEls   = [ImpCharacter () u "a", ImpCharacter () u "b", ImpCharacter () u "c"]
          ...
          imp1 =
            ImpList ()
                    u
                    (TypeSpec () u (TypeCharacter (Just $ CharLenInt 30) Nothing) (Just selector))
              $ AList () u impEls
          imp2 = ImpList () u (TypeSpec () u TypeInteger Nothing)
            $ AList () u [ImpRange () u "a" "z", ImpCharacter () u "l"]
  ...
  it "parses 'parameter (pi = 3.14, b = 'X' // 'O', d = k) '" $ do
    let sts =
          [ DeclVariable () u (varGen "pi") Nothing (Just $ realGen (3.14 :: Double))
  ...
  describe "SAVE" $ do
    ...
    it "parses 'save'" $ sParser "      save" `shouldBe'` StSave () u Nothing
  ...
  describe "Legacy Extensions" $ do
    ...
    it "parses character declarations with unspecfied lengths" $ do
      let src = "      character s*(*)"
          st =
            StDeclaration () u (TypeSpec () u (TypeCharacter Nothing Nothing) Nothing) Nothing
              $ AList
                  ()
                  u
                  [ DeclVariable ()
    ...
    it "parses array initializers" $ do
      let
        src = "      integer xs(3) / 1, 2, 3 /"
        inits =
          [ ExpValue () u (ValInteger "1")
          , ExpValue () u (ValInteger "2")
          , ExpValue () u (ValInteger "3")
          ]
        st = StDeclaration () u (TypeSpec () u TypeInteger Nothing) Nothing $ AList
          ()
          u
          [ DeclArray
              ()
      ...
      let
        src1   = "      character xs(2)*5 / 'hello', 'world' /"
        ...
        st1 =
          StDeclaration () u (TypeSpec () u (TypeCharacter Nothing Nothing) Nothing) Nothing $ AList
            ...
            [ DeclArray
                ...
                (AList ()
                       u
                       [DimensionDeclarator () u Nothing (Just (ExpValue () u (ValInteger "2")))]
                )
      ...
      let
        src2   = "      character xs*5(2) / 'hello', 'world' /"
    ...
    it "parses subscripts in assignments" $ do
      let
        mkIdx i = IxSingle () u Nothing (ExpValue () u (ValInteger i))

        src = "      x(0,1) = 0"
        tgt =
          ExpSubscript () u (ExpValue () u (ValVariable "x")) (AList () u [mkIdx "0", mkIdx "1"])
      ...
      let src1 = "      x(0).foo = 0"
          tgt1 = ExpDataRef
            ()
            u
            (ExpSubscript () u (ExpValue () u (ValVariable "x")) (AList () u [mkIdx "0"]))
            (ExpValue () u (ValVariable "foo"))
          st1  = StExpressionAssign () u tgt1 (ExpValue () u (ValInteger "0"))
      resetSrcSpan (slParser src1) `shouldBe` st1

      let src2 = "      x.foo = 0"
          tgt2 =
            ExpDataRef () u (ExpValue () u (ValVariable "x")) (ExpValue () u (ValVariable "foo"))
...
exampleProgram1 = unlines ["      program hello", "      integer x", "      end"]

exampleProgram2 :: String
exampleProgram2 = unlines ["      block data hello", "      integer x", "      end"]
