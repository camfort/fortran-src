...
spec :: Spec
spec = describe "Fortran 95 Parser" $ do
  describe "Function" $ do
    ...
    describe "End" $ do
      ...
      it "parses functions with return type specs" $ do
        let fType'   = Just $ TypeSpec () u TypeInteger Nothing
            ...
            fStr     = init $ unlines ["integer function f()", "end function f"]
        fParser fStr `shouldBe'` expected
    ...
    describe "parses function options (recursive, pure, elemental)" $ do
      ...
      forM_
        ...
        (\(strs, opts) -> do
          ...
          case opt of
            Left _ ->
              it ("Shouldn't parse: " ++ show fStr ++ ": " ++ show opt)
                $             evaluate (fParser fStr)
                `shouldThrow` anyIOException
            Right fOpt' -> it ("Should parse: " ++ show fStr ++ ": " ++ show opt) $ do
              let expected' = puFunction fType fOpt' fName fArgs fRes fBody fSub
              fParser fStr `shouldBe'` expected'
        )
    ...
    it "parses functions with a list of arguments" $ do
      let fArgs'   = Just $ AList () u [varGen "x", varGen "y", varGen "z"]
          expected = puFunction fType fOpt fName fArgs' fRes fBody fSub
          fStr     = init $ unlines ["function f(x, y, z)", "end function f"]
      fParser fStr `shouldBe'` expected

    it "parses functions with a result variable" $ do
      let fRes'    = Just $ varGen "i"
          expected = puFunction fType fOpt fName fArgs fRes' fBody fSub
          fStr     = init $ unlines ["function f() result(i)", "end function f"]
      fParser fStr `shouldBe'` expected
    ...
    it "parses functions with function bodies" $ do
      let decrementRHS = ExpBinary () u Subtraction (varGen "i") (intGen 1)
          ...
          expected     = puFunction fType fOpt fName fArgs fRes fBody' fSub
          fStr         = init $ unlines ["function f()", "  print *, i", "  i = (i - 1)", "end function f"]
      fParser fStr `shouldBe'` expected

    it "parses complex functions" $ do
      let
        fType'       = Just $ TypeSpec () u TypeInteger Nothing
  ...
  describe "Expression" $ do
    ...
    describe "Custom operator" $ do
      ...
      it "parses unary custom operator" $ eParser ".inverse. 42" `shouldBe'` unExp
  ...
  describe "Statement" $ do
    ...
    it "parses declaration with attributes" $ do
      let
        typeSpec    = TypeSpec () u TypeReal Nothing
        attrs       = AList
          ()
          u
          [ AttrExternal () u
          , AttrIntent () u Out
          , AttrDimension () u
            $ AList () u [DimensionDeclarator () u (Just $ intGen 3) (Just $ intGen 10)]
          ]
        declarators = AList
          ()
          u
    ...
    it "parses declaration with initialisation" $ do
      let typeSpec    = TypeSpec () u TypeComplex Nothing
          ...
          declarators = AList () u [DeclVariable () u (varGen "x") Nothing (Just init')]
          expected    = StDeclaration () u typeSpec Nothing declarators
    ...
    it "parses declaration of custom type" $ do
      let typeSpec    = TypeSpec () u (TypeCustom "meinetype") Nothing
          declarators = AList () u [DeclVariable () u (varGen "x") Nothing Nothing]
          expected    = StDeclaration () u typeSpec Nothing declarators
    ...
    it "parses private statement" $ sParser "private" `shouldBe'` StPrivate () u Nothing
    ...
    describe "Data" $ do
      ...
      describe "Delimeter" $ do
        let [nlist1, vlist1] = map (fromList () . return) [varGen "x", intGen 1]
            [nlist2, vlist2] = map (fromList () . return) [varGen "y", intGen 2]
            list             = [DataGroup () u nlist1 vlist1, DataGroup () u nlist2 vlist2]
            expected         = StData () u (fromList () list)

        it "parses comma delimited init groups" $ sParser "data x/1/, y/2/" `shouldBe'` expected

        it "parses non-comma delimited init groups" $ sParser "data x/1/ y/2/" `shouldBe'` expected

    describe "Namelist" $ do
      let groupNames =
            [ExpValue () u (ValVariable "something"), ExpValue () u (ValVariable "other")]
          itemss = [fromList () [varGen "a", varGen "b", varGen "c"], fromList () [varGen "y"]]
          st     = StNamelist () u $ fromList
            ()
            [ Namelist () u (head groupNames) (head itemss)
            , Namelist () u (last groupNames) (last itemss)
            ]

      it "parses namelist statement (comma delimited) (1)"
        $           sParser "namelist /something/a,b,c,/other/y"
        `shouldBe'` st
    ...
    describe "Common" $ do
      ...
      it "parses common statement (comma delimited) (1)"
        $           sParser "common q /something/a,b,c, /other/y"
        `shouldBe'` st

      it "parses common statement (2)" $ sParser "common q /something/a,b,c /other/y" `shouldBe'` st
      ...
      let
        eqALists = fromList
          ...
          , fromList
            ...
            , varGen "z"
            , let indicies =
                    fromList () [IxRange () u (Just $ intGen 1) (Just $ intGen 42) Nothing]
    ...
    describe "Dynamic allocation" $ do
      it "parses allocate statement" $ do
        let controlPair = ControlPair () u (Just "stat") (varGen "a")
            allocs      = fromList () [varGen "x", ExpDataRef () u (varGen "st") (varGen "part")]
            s           = StAllocate () u allocs (Just controlPair)
    ...
    describe "Where" $ do
      ...
      describe "Where block" $ do
        it "parses where construct statement"
          $           sParser "where (.true.)"
          `shouldBe'` StWhereConstruct () u valTrue

        it "parses elsewhere statement" $ sParser "elsewhere" `shouldBe'` StElsewhere () u

        it "parses endwhere statement" $ sParser "endwhere" `shouldBe'` StEndWhere () u

  describe "If" $ do
    it "parses if-then statement"
      $           sParser "if (.false.) then"
      `shouldBe'` StIfThen () u Nothing valFalse
    ...
    it "parses else statement" $ sParser "else" `shouldBe'` StElse () u Nothing

    it "parses else-if statement"
      $           sParser "else if (.true.) then"
      `shouldBe'` StElsif () u Nothing valTrue

    it "parses end if statement" $ sParser "end if" `shouldBe'` StEndif () u Nothing
    ...
    it "parses arithmetic if statement" $ do
      let stIf = StIfArithmetic () u (varGen "x") (intGen 1) (intGen 2) (intGen 3)
  ...
  describe "Case" $ do
    ...
    it "parses case statement" $ sParser "case default" `shouldBe'` StCase () u Nothing Nothing
  ...
  describe "Goto" $ do
    ...
    it "doesn't parse assigned goto"
      $             evaluate (sParser "goto i, (10, 20, 30)")
      `shouldThrow` anyIOException

    it "doesn't parse label assignment"
      $             evaluate (sParser "assign 20 to l")
      `shouldThrow` anyIOException
