...
spec :: Spec
spec = describe "Fortran 66 Parser" $ do
  describe "Expressions" $ do
    describe "Arithmetic expressions" $ do
      ...
      it "parses '3 + -2 + 42'" $ do
        let expectedExp = ExpBinary
              ()
              u
              Addition
              (ExpBinary () u Addition (intGen 3) (ExpUnary () u Minus (intGen 2)))
              (intGen 42)
      ...
      it "parses '3 + 4 * 12'" $ do
        let expectedExp = ExpBinary ()
                                    ...
                                    Addition
                                    (intGen 3)
                                    (ExpBinary () u Multiplication (intGen 4) (intGen 12))
        eParser "3 + 4 * 12" `shouldBe'` expectedExp

    describe "Logical expressions" $ it "parses '.true. .and. .false.'" $ do
      let expectedExp = ExpBinary () u And valTrue valFalse
      eParser ".true. .and. .false." `shouldBe'` expectedExp

    describe "Relational expressions" $ it "parses '(3 * 2) .lt. 42'" $ do
      let expectedExp =
            ExpBinary () u LT (ExpBinary () u Multiplication (intGen 3) (intGen 2)) (intGen 42)
      eParser "(3 * 2) .lt. 42" `shouldBe'` expectedExp

    describe "Other expressions" $ it "parses 'a(2 * x - 3, 10)'" $ do
      let firstEl     = ExpBinary ()
                                  u
                                  Subtraction
                                  (ExpBinary () u Multiplication (intGen 2) (varGen "x"))
                                  (intGen 3)
          expectedExp = ExpSubscript ()
                                     u
                                     (varGen "a")
                                     (AList () u [IxSingle () u Nothing firstEl, ixSinGen 10])
  ...
  describe "Statements" $ do
    ...
    it "parses 'COMMON // a, b /hello/ x, y, z'" $ do
      let comGrs =
            [ CommonGroup () u Nothing (AList () u [varGen "a", varGen "b"])
            , CommonGroup ()
                          u
                          (Just $ varGen "hello")
    ...
    it "parses 'EQUIVALENCE (a,b), (x,y,z)'" $ do
      let ls =
            [AList () u [varGen "a", varGen "b"], AList () u [varGen "x", varGen "y", varGen "z"]]
      let st = StEquivalence () u (AList () u ls)
      sParser "      EQUIVALENCE (a,b), (x,y,z)" `shouldBe'` st

    it "parses 'DATA a/1,2,3/,x/42/'" $ do
      let dGrs =
            [ DataGroup () u (AList () u [varGen "a"]) (AList () u [intGen 1, intGen 2, intGen 3])
    ...
    it "parses 'integer i, j(2,2), k'" $ do
      let
        dimDecls = replicate 2 $ DimensionDeclarator () u Nothing (Just $ intGen 2)
        declarators =
          [ DeclVariable () u (varGen "i") Nothing Nothing
          , DeclArray () u (varGen "j") (AList () u dimDecls) Nothing Nothing
          , DeclVariable () u (varGen "k") Nothing Nothing
          ]
        st =
          StDeclaration () u (TypeSpec () u TypeInteger Nothing) Nothing $ AList () u declarators
      sParser "      integer i, j(2,2), k" `shouldBe'` st

    let controlPairs =
          AList () u [ControlPair () u Nothing (intGen 6), ControlPair () u Nothing (labelGen 10)]
    ...
    describe "WRITE" $ do
      ...
      it "parses 'write (6) i'" $ do
        let expectedSt = StWrite ()
                                 u
                                 (AList () u [ControlPair () u Nothing (intGen 6)])
                                 (Just $ AList () u [varGen "i"])
        sParser "      write (6) i" `shouldBe'` expectedSt

      it "parses 'write (6,10) i'" $ sParser "      write (6,10) i" `shouldBe'` writeSt
