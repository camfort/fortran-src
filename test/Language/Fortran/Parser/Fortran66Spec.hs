    ...
    describe "Statements" $ do
      ...
      it "parses 'integer i, j(2,2), k'" $ do
        let dimDecls = replicate 2 $ DimensionDeclarator () u Nothing (Just $ intGen 2)
            declarators = [ Declarator () u (varGen "i") Nothing Nothing Nothing
                          , Declarator () u (varGen "j") (Just (AList () u dimDecls)) Nothing Nothing
                          , Declarator () u (varGen "k") Nothing Nothing Nothing ]
