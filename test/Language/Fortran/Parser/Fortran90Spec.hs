...
spec :: Spec
spec = describe "Fortran 90 Parser" $ do
  describe "Function" $ do
    ...
    describe "End" $ do
      ...
      it "parses functions with return type specs" $ do
        ...
        let expected = puFunction fType' fOpt fName fArgs fRes fBody fSub
        let fStr     = init $ unlines ["integer function f()", "end function f"]
        fParser fStr `shouldBe'` expected
  ...
  describe "Statement" $ do
    ...
    it "parses declaration of custom type" $ do
      ...
      let declarators = AList () u [DeclVariable () u (varGen "x") Nothing Nothing]
      let expected    = StDeclaration () u typeSpec Nothing declarators
    ...
    describe "Data" $ do
      ...
      describe "Delimeter" $ do
        ...
        let [nlist2, vlist2] = map (fromList () . return) [varGen "y", intGen 2]
        let list             = [DataGroup () u nlist1 vlist1, DataGroup () u nlist2 vlist2]
        let expected         = StData () u (fromList () list)
    ...
    describe "Namelist" $ do
      ...
      let itemss = [fromList () [varGen "a", varGen "b", varGen "c"], fromList () [varGen "y"]]
      let st = StNamelist () u $ fromList
            ()
    ...
    describe "Dynamic allocation" $ do
      it "parses allocate statement" $ do
        ...
        let allocs      = fromList () [varGen "x", ExpDataRef () u (varGen "st") (varGen "part")]
