...
import qualified Language.Fortran.Parser.Fortran90
                                               as F90
...
pParser version source =
  rename . analyseBBlocks . analyseRenames . initAnalysis . resetSrcSpan $ parser version
                                                                                  source
                                                                                  "<unknown>"
...
testPfAndGraph :: Parser t
               => t
               -> String
               -> String
               -> (ProgramFile (Analysis A0), BBGr (Analysis A0))
testPfAndGraph version f p =
  fmap (fromJust . M.lookup (Named f)) . withParse version p $ \pf -> (pf, genBBlockMap pf)

testGenDefMap :: Parser t => t -> String -> DefMap
testGenDefMap version =
  flip (withParse version) (genDefMap . genBlockMap . analyseBBlocks . initAnalysis)
...
spec :: Spec
spec = describe "Dataflow" $ do
  describe "loop4" $ do
    it "genBackEdgeMap" $ do
      let gr = testGraph F77 "loop4" programLoop4
      testBackEdges F77 "loop4" programLoop4 `shouldBe` IM.fromList
        [(findLabelBB gr 8, findLabelBB gr 10), (findLabelBB gr 7, findLabelBB gr 20)]
    ...
    it "loopNodes" $ do
      let pf     = pParser F77 programLoop4
          ...
          domMap = dominators gr
          bedges = genBackEdgeMap domMap gr
      S.fromList (loopNodes bedges gr) `shouldBe` S.fromList
        [ findLabelsBB gr [5, 6, 7, 20]
        , IS.unions [findLabelsBB gr [4, 5, 6, 7, 8, 10, 20, 30], findSuccsBB gr [20]]
        ]

    it "genDefMap" $ do
      let pf = pParser F77 programLoop4
      testGenDefMap F77 programLoop4 `shouldBe` M.fromList
        [ ("i", findLabelsBl pf [3, 30])
        , ("j", findLabelsBl pf [4, 6])
        , ("r", findLabelsBl pf [2, 5])
        ]
    ...
    it "reachingDefinitions" $ do
      let pf = pParser F77 programLoop4
          ...
          bm = genBlockMap pf
          dm = genDefMap bm
      IM.lookup (findLabelBB gr 5) (reachingDefinitions dm gr)
        `shouldBe` Just (findLabelsBl pf [2, 3, 4, 5, 6, 30], findLabelsBl pf [3, 4, 5, 6, 30])
    ...
    it "flowsTo" $ do
      let pf = pParser F77 programLoop4
          ...
          bm = genBlockMap pf
          dm = genDefMap bm
      (S.fromList . edges . genFlowsToGraph bm dm gr $ reachingDefinitions dm gr)
        `shouldBe`
...
----------------------------------------------
  ...
  describe "loop4 alt (module)" $ do
    it "genBackEdgeMap" $ do
      ...
      testBackEdges F90 "loop4" programLoop4Alt `shouldBe` IM.fromList
        [(findLabelBB gr 22, findLabelBB gr 20), (findLabelBB gr 31, findLabelBB gr 10)]
    ...
    it "loopNodes" $ do
      let pf     = pParser F90 programLoop4Alt
          ...
          bedges = genBackEdgeMap domMap gr
      S.fromList (loopNodes bedges gr) `shouldBe` S.fromList
        [findLabelsBB gr [20, 21, 22], findLabelsBB gr [10, 11, 20, 21, 22, 31, 40]]

    it "genDefMap" $ do
      let pf = pParser F90 programLoop4Alt
      testGenDefMap F90 programLoop4Alt `shouldBe` M.fromList
        [ ("i", findLabelsBl pf [2, 31])
        , ("j", findLabelsBl pf [11, 22])
        , ("r", findLabelsBl pf [1, 21])
        ]
    ...
    it "reachingDefinitions" $ do
      let pf  = pParser F90 programLoop4Alt
          ...
          bm  = genBlockMap pf
          dm  = genDefMap bm
      IM.lookup (findLabelBB gr 21) (reachingDefinitions dm gr) `shouldBe` Just
        (findLabelsBl pf [1, 2, 11, 21, 22, 31], findLabelsBl pf [2, 11, 21, 22, 31])
  ...
  describe "rd3" $ do
    ...
    it "loopNodes" $ do
      let (_, gr) = testPfAndGraph F77 "f" programRd3
          domMap  = dominators gr
          bedges  = genBackEdgeMap domMap gr
      S.fromList (loopNodes bedges gr) `shouldBe` S.fromList [findLabelsBB gr [1, 2, 3, 4]]

    it "reachingDefinitions" $ do
      let (pf, gr) = testPfAndGraph F77 "f" programRd3
          bm       = genBlockMap pf
          dm       = genDefMap bm
      IM.lookup (findLabelBB gr 5) (reachingDefinitions dm gr) `shouldBe` Just
        ( IS.unions [findBBlockBl gr 0, findLabelsBl pf [1, 2, 3]]
  ...
  describe "rd4" $ it "ivMapByASTBlock" $ do
    let (_, gr) = testPfAndGraph F77 "f" programRd4
        domMap  = dominators gr
        bedges  = genBackEdgeMap domMap gr
        ivMap   = genInductionVarMapByASTBlock bedges gr
    (sort . map (head &&& length) . group . sort . map S.size $ IM.elems ivMap)
      `shouldBe` [(1, 3), (2, 3)]
  ...
  describe "bug36" $ do
    let pf     = pParser F90 programBug36
        ...
        domMap = dominators gr
        bedges = genBackEdgeMap domMap gr
    it "loopNodes" $ length (loopNodes bedges gr) `shouldBe` 2
  ...
  describe "funcflow1" $ do
    ...
    it "flowsTo" $ (S.fromList . edges . trc $ flTo) `shouldSatisfy`
        -- Find the flows of the assignment statements in the program.
                                                                     S.isSubsetOf
      (findLabelsBlEdges pf [(1, 2), (1, 3), (3, 2)])
  ...
  describe "funcflow2" $ do
    let pf     = pParser F90 programFuncFlow2
        ...
        (iLabel, iName) : _ =
          [ (fromJust (insLabel a), varName e)
          | e@(ExpValue a _ (ValVariable _)) <- rhsExprs pf
          , srcName e == "i"
          ]
        (jLabel, _) : _ =
          [ (fromJust (insLabel a), varName e)
          | e@(ExpValue a _ (ValVariable _)) <- lhsExprs pf
          , srcName e == "j"
          ]
    it "flowsTo" $ (S.fromList . edges . trc $ flTo) `shouldSatisfy`
        -- Find the flows of the assignment statements in the program.
  ...
  describe "defUse1" $ do
    let pf     = pParser F90 programDefUse1
        ...
        domMap = dominators gr
        bedges = genBackEdgeMap domMap gr
    it "backEdges" $ bedges `shouldBe` IM.fromList [(findLabelBB gr 5, findLabelBB gr 4)]
    it "flowsTo" $ (S.fromList . edges $ flTo) `shouldBe`
        -- Find the flows of the assignment statements in the program.
                                                          findLabelsBlEdges
      pf
      [(1, 2), (1, 3), (1, 5), (2, 3), (3, 4), (4, 5), (5, 5)]
  ...
  describe "defUse2" $ do
    let pf     = pParser F90 programDefUse2
        ...
        domMap = dominators gr
        bedges = genBackEdgeMap domMap gr
    it "backEdges" $ bedges `shouldBe` IM.fromList [(findLabelBB gr 12, findLabelBB gr 11)]
    it "flowsTo" $ (S.fromList . edges $ flTo) `shouldSatisfy`
        -- Find the flows of the assignment statements in the program.
                                                               S.isSubsetOf
      (findLabelsBlEdges pf [(1, 2), (1, 3), (1, 4), (2, 3), (3, 11), (4, 4), (11, 12), (12, 12)])
  ...
  describe "other"
    ...
    $          dominators
                 (nmap
                   (const [])
                   (mkUGraph [0, 1, 3, 4, 5, 6, 7, 8, 9]
                             [(0, 3), (3, 1), (5, 6), (6, 7), (7, 4), (7, 8), (8, 7), (8, 9), (9, 8)] :: Gr
                       ()
                       ()
                   )
                 )
    `shouldBe` IM.fromList
                 ...
                 , (1, IS.fromList [0, 1, 3])
                 , (3, IS.fromList [0, 3])
                 , (4, IS.fromList [4, 5, 6, 7])
                 , (5, IS.fromList [5])
                 , (6, IS.fromList [5, 6])
                 , (7, IS.fromList [5, 6, 7])
                 , (8, IS.fromList [5, 6, 7, 8])
                 , (9, IS.fromList [5, 6, 7, 8, 9])
                 ]
...
findLabelsBl pf labs = IS.fromList
  [ i
  | b                              <- universeBi pf :: [Block (Analysis a)]
 ...
 where
  convEdge (a, b)
    | a' : _ <- IS.toList (findLabelsBl pf [a]) -- FIXME: inefficient
                                               , b' : _ <- IS.toList (findLabelsBl pf [b])
    = (a', b')
    | otherwise
    = error $ "findLabelsBlEdges (" ++ show a ++ "," ++ show b ++ ")"
