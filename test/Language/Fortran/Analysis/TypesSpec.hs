...
spec = do
  ...
  describe "Local type inference" $ do
    ...
    describe "Intrinsics type analysis"
      $ it "disambiguates intrinsics from functions and variables"
      $ do
    ...
    describe "Character string types" $ it "examples of various character variables" $ do
...
ex4pu1bs =
    ...
    (StDeclaration
      ...
      (AList
        ...
        , DeclArray ()
                    u
                    (varGen "y")
                    (AList () u [DimensionDeclarator () u Nothing (Just $ intGen 10)])
                    Nothing
                    Nothing
        ]
      )
  ...
  , BlStatement
    ...
    (StDeclaration ()
                   u
                   (TypeSpec () u (TypeCharacter Nothing Nothing) Nothing)
                   Nothing
  ...
  , BlStatement
    ...
    (StDeclaration ()
                   u
                   (TypeSpec () u TypeLogical Nothing)
                   Nothing
...
ex6pu1bs =
  ...
  , BlStatement
    ...
    u
    Nothing
    (StExpressionAssign () u (ExpSubscript () u (varGen "a") (fromList () [ixSinGen 1])) (intGen 1))
  , BlStatement
    ()
    u
    Nothing
    (StExpressionAssign () u (ExpSubscript () u (varGen "b") (fromList () [ixSinGen 1])) (intGen 1))
  , BlStatement
    ()
    u
    Nothing
    (StExpressionAssign () u (ExpSubscript () u (varGen "c") (fromList () [ixSinGen 1])) (intGen 1))
  , BlStatement
    ()
    u
    Nothing
    (StExpressionAssign () u (ExpSubscript () u (varGen "d") (fromList () [ixSinGen 1])) (intGen 1))
  ]
