...
pParser source =
  rename . analyseBBlocks . analyseRenames . initAnalysis . fromParseResultUnsafe $ extended77Parser
    (B.pack source)
    "<unknown>"

spec :: Spec
spec = describe "Basic Blocks" $ do
  describe "loop4" $ do
  ...
  describe "if arith" $ do
    ...
    it "branching nodes" $ do
      let pf = pParser programArithIf
      let gr = fromJust . M.lookup (Named "arithif") $ genBBlockMap pf
      (IS.size (findSuccsBB gr [10]), IS.size (findSuccsBB gr [20]), IS.size (findSuccsBB gr [30]))
        `shouldBe` (1, 1, 1)
    it "all reachable" $ do
      let pf      = pParser programArithIf
      let gr      = fromJust . M.lookup (Named "arithif") $ genBBlockMap pf
